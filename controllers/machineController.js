...
export const getMachineById = asyncHandler(async (req, res) => {
  ...
  const { id } = req.params;
  const machine = await Machine.findOne({ serialNumber: id }).lean();
  if (!machine) {
    res.status(404).json({ success: false, message: "not found machine with this Serial Number" });
    return;
  }
...
export const updateMachineById = asyncHandler(async (req, res) => {
...
export const deleteOneMachineById = asyncHandler(async (req, res) => {
  const machine = await Machine.findByIdAndDelete(req.params.id);
  if (!machine) {
    res.status(404);
    throw new Error("not found machine with this ID ");
  }
  await Visit.deleteOne({ machineId: req.params.id });
  res.status(200).json({
    success: true,
...
export const deleteAllData = asyncHandler(async (req, res) => {
  ...
  // cheking if there was any data to delete (optional)
  if (deletedMachines.deletedCount === 0 && deletedVisits.deletedCount === 0) {
    return res.status(404).json({
      success: false,
      message: "No machines or visits found to delete",
    });
  }
  res.status(200).json({
    success: true,
